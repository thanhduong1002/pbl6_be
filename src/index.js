...
app.post("/insertFeedback", async (req, res) => {
  const test = new Feedback(req.body);
  test
    .save()
    .then(() => {
      res.send(test);
...
app.get("/getall", async (req, res) => {
  ...
  response.status = 200;
  response.message = feedbacks;
  response.internal_message = `success with ${feedbacks}`;
...
app.post("/search", async (req, res) => {
  ...
  const news = await New.find({ author: search });
  response.status = 200;
  response.message = news;
  response.internal_message = `success with ${search}`;
